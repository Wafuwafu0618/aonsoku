...
struct ImpulseAnalysisMetrics {
    ...
    impulse_plot_path: Option<String>,
}
...
fn write_impulse_spectrum_svg(
    ...
    if channels == 0 {
        return Err("Impulse plot requires at least one channel.".to_string());
    }
    if output_sample_rate_hz == 0 {
        return Err("Impulse plot requires valid output sample rate.".to_string());
    }
    if samples.len() < channels {
        return Err("Impulse plot requires non-empty sample data.".to_string());
    }

    let frames = samples.len() / channels;
    if frames == 0 {
        return Err("Impulse plot requires at least one frame.".to_string());
    }

    let fft_size = frames.next_power_of_two().max(2048);
    if fft_size > MAX_IMPULSE_FFT_SIZE {
        return Err(format!(
            "Impulse response is too long for plot generation ({} frames > supported limit {}).",
            frames, MAX_IMPULSE_FFT_SIZE
        ));
    }
    ...
    let half_bins = fft_size / 2;
    if half_bins == 0 {
        return Err("Impulse plot requires FFT with at least 2 bins.".to_string());
    }

    let width_px = 1600.0_f64;
    let height_px = 900.0_f64;
    let margin_left = 90.0_f64;
    let margin_right = 30.0_f64;
    let margin_top = 40.0_f64;
    let margin_bottom = 70.0_f64;
    let plot_width = width_px - margin_left - margin_right;
    let plot_height = height_px - margin_top - margin_bottom;
    let db_min = -180.0_f64;
    let db_max = 24.0_f64;
    let bin_hz = output_sample_rate_hz as f64 / fft_size as f64;

    let target_points = 3000usize;
    let step = ((half_bins + 1).saturating_add(target_points - 1)) / target_points;
    let mut path_data = String::new();
    let mut first = true;

    for start_bin in (0..=half_bins).step_by(step.max(1)) {
        let end_bin = (start_bin + step.max(1)).min(half_bins + 1);
        ...
        let db = dbfs_from_amplitude(peak_mag).clamp(db_min, db_max);
        let x = margin_left + (peak_bin as f64 / half_bins as f64) * plot_width;
        let y = margin_top + ((db_max - db) / (db_max - db_min)) * plot_height;
        if first {
    ...
    let mut vertical_grid = String::new();
    let nyquist_hz = output_sample_rate_hz as f64 * 0.5;
    for index in 0..=10 {
        let ratio = index as f64 / 10.0;
        let x = margin_left + ratio * plot_width;
        let hz = nyquist_hz * ratio;
        let label = if hz >= 1000.0 {
            format!("{:.1}k", hz / 1000.0)
        } else {
            format!("{:.0}", hz)
        };
        vertical_grid.push_str(&format!(
            "<line x1=\"{x:.2}\" y1=\"{y1:.2}\" x2=\"{x:.2}\" y2=\"{y2:.2}\" stroke=\"#e4e7eb\" stroke-width=\"1\" />\n\
             <text x=\"{x:.2}\" y=\"{text_y:.2}\" text-anchor=\"middle\" font-size=\"12\" fill=\"#495057\">{label}</text>\n",
            y1 = margin_top,
            y2 = margin_top + plot_height,
            text_y = margin_top + plot_height + 22.0
        ));
    }

    let mut horizontal_grid = String::new();
    let mut db = db_max as i32;
    while db >= db_min as i32 {
        let ratio = (db_max - db as f64) / (db_max - db_min);
        let y = margin_top + ratio * plot_height;
        horizontal_grid.push_str(&format!(
            "<line x1=\"{x1:.2}\" y1=\"{y:.2}\" x2=\"{x2:.2}\" y2=\"{y:.2}\" stroke=\"#f0f2f5\" stroke-width=\"1\" />\n\
             <text x=\"{text_x:.2}\" y=\"{text_y:.2}\" text-anchor=\"end\" font-size=\"12\" fill=\"#495057\">{db} dB</text>\n",
            x1 = margin_left,
            x2 = margin_left + plot_width,
            text_x = margin_left - 8.0,
            text_y = y + 4.0
        ));
        db -= 20;
    }

    let escaped_title = escape_svg_text(title);
    let svg = format!(
        "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"{width}\" height=\"{height}\" viewBox=\"0 0 {width} {height}\">\n\
         <rect width=\"100%\" height=\"100%\" fill=\"#ffffff\" />\n\
         <text x=\"{title_x:.2}\" y=\"26\" text-anchor=\"middle\" font-size=\"20\" font-family=\"Segoe UI, Arial, sans-serif\" fill=\"#111\">{title}</text>\n\
         {vertical_grid}{horizontal_grid}\
         <rect x=\"{plot_x:.2}\" y=\"{plot_y:.2}\" width=\"{plot_w:.2}\" height=\"{plot_h:.2}\" fill=\"none\" stroke=\"#ced4da\" stroke-width=\"1.2\" />\n\
         <path d=\"{path}\" fill=\"none\" stroke=\"#0d6efd\" stroke-width=\"1.8\" stroke-linejoin=\"round\" stroke-linecap=\"round\" />\n\
         <text x=\"{x_label_x:.2}\" y=\"{x_label_y:.2}\" text-anchor=\"middle\" font-size=\"13\" fill=\"#212529\">Frequency (Hz)</text>\n\
         <text transform=\"translate({y_label_x:.2},{y_label_y:.2}) rotate(-90)\" text-anchor=\"middle\" font-size=\"13\" fill=\"#212529\">Magnitude (dBFS)</text>\n\
         </svg>\n",
        width = width_px as i32,
        height = height_px as i32,
        title_x = width_px / 2.0,
        title = escaped_title,
        ...
        plot_h = plot_height,
        path = path_data,
        x_label_x = margin_left + plot_width / 2.0,
        x_label_y = height_px - 20.0,
        y_label_x = 24.0,
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .map_err(|error| format!("Failed to create plot output directory: {error}"))?;
    }
    fs::write(path, svg).map_err(|error| format!("Failed to write impulse plot SVG: {error}"))?;
    Ok(())
}
...
fn parse_args_from_values(args: &[String], exit_on_help: bool) -> Result<CliArgs, String> {
    ...
    let mut write_impulse_plot = false;
    ...
    if write_impulse_plot && !analyze_impulse {
        return Err("--write-impulse-plot requires --analyze-impulse.".to_string());
    }
    if write_impulse_wav && output_dir.is_none() {
        return Err("--output-dir is required when --write-impulse-wav is enabled.".to_string());
    }
    if write_impulse_plot && output_dir.is_none() {
        return Err("--output-dir is required when --write-impulse-plot is enabled.".to_string());
    }
...
fn parse_args() -> Result<CliArgs, String> {
    ...
    parse_args_from_values(&args, true)
}

fn parse_args_from_tokens(tokens: &[String]) -> Result<CliArgs, String> {
    parse_args_from_values(tokens, false)
}

fn run_with_parsed_args(args: CliArgs) -> Result<OfflineLabReport, String> {
    ...
    if args.analyze_impulse {
        ...
        for filter in &args.filters {
            ...
            let impulse_plot_path = if args.write_impulse_plot {
                let output_dir = args.output_dir.as_ref().ok_or_else(|| {
                    "--output-dir is required when --write-impulse-plot is enabled.".to_string()
                })?;
                let file_name = format!("impulse-spectrum-{}.svg", sanitize_case_name(filter));
                let path = output_dir.join(file_name);
                let plot_title = format!(
                    "Impulse Spectrum: {} @ {} Hz",
                    filter, impulse_case.output_sample_rate_hz
                );
                write_impulse_spectrum_svg(
                    &path,
                    &impulse_case.samples,
                    impulse_case.channels as usize,
                    impulse_case.output_sample_rate_hz,
                    &plot_title,
                )?;
                Some(path.to_string_lossy().to_string())
...
pub fn run_cli() -> Result<(), String> {
    let args = parse_args()?;
    let report = run_with_parsed_args(args)?;
...
pub fn run_from_tokens(tokens: &[String]) -> Result<String, String> {
    let args = parse_args_from_tokens(tokens)?;
    let report = run_with_parsed_args(args)?;
