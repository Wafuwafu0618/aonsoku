...
## WP5. UI とキューを source-aware 化 ✅ 完了
...
## WP6. Electron / Desktop 依存の adapter 化 ✅ 完了
...
これは後続の Spotify Connect、内蔵オーバーサンプラー（CPU/GPU）、排他モード対応のための重要な下地になる。
...
**主要成果物**:
- `src/domain/**` - source-aware domain model（Track, Album, Artist, QueueItem）
- `src/playback/**` - PlaybackBackend abstraction（interface + InternalPlaybackBackend）
- `src/store/playback-session.store.ts` - session state分離
- `src/store/player-controller.ts` - 副作用分離（scrobble, Discord RPC, Desktop連携）
- `src/platform/**` - Desktop API adapter層（7アダプター）
- `src/app/components/source-badge.tsx` - Sourceバッジコンポーネント

**次フェーズ**: Phase 3 - ローカルライブラリ共存
